...
/// Run some cleanup code when the component is unmounted if the effect was run.
pub fn use_effect_with_cleanup<F: FnMut() -> C + 'static, C: FnOnce() + 'static>(mut effect: F) {
...
pub fn use_global_keydown_listener(key: &'static str, on_escape: impl FnMut() + Clone + 'static) {
